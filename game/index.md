...
::: info A Corium side project
These pages cover **Corium Game**, the on-chain game of chance that runs at [www.corium.so](https://www.corium.so) today. The main Corium product is the [memecoin launchpad](/guide/what-is-corium).
:::

Corium Game is a game of chance on Solana, played inside a universe that its players build together.
...
| | |
|---|---|
| **Worlds** | Every feed that lands becomes a planet in that star's system. Nine orbital zones, nine planet families, and a rarity tier for each. [The universe →](/game/universe) |
| **Remnants** | Kill a star and its remnant is yours, the rarest thing on a profile card. |
| **A profile** | A callsign, a rank, the world you choose to wear, and your record, public for anyone to visit. [Ranks & perks →](/game/progression) |
...
**The house can't touch the rules.** Odds, fee and thresholds are compiled into the program. There is no admin instruction, no pause, and no setting to change. The deployed build is verified against the public source. See [the trust model](/game/litepaper#trust-model) for the one thing you do have to trust.
...
**Next:** [Getting started →](/game/getting-started)
