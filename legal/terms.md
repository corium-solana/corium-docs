# Terms of use

**Version 1.** This page mirrors the terms you agree to when you sign in to the Corium launchpad. The in-app version is authoritative.

::: info The short version
- You are 18 or older and allowed to use a service like this where you live.
- Memecoins are extremely risky. Most go to zero. Only trade what you can lose.
- Corium is software. Nothing here is advice, and Corium does not hold your funds.
- Coins and chat are posted by users. Corium does not endorse them.
- No scams, impersonation, harassment or illegal content. We hide it and ban wallets.
:::

These terms govern your use of Corium (corium.so), an interface for launching and trading tokens on Solana through third-party smart contracts, including Meteora's Dynamic Bonding Curve and DAMM v2 pools and the Corium bounty distributor. By signing in with your wallet you agree to them.

## Eligibility

You must be at least 18 years old and legally able to enter into these terms. You may not use Corium if you are located in, or a resident of, a jurisdiction where using it is prohibited, or if you are subject to sanctions. You are responsible for complying with the laws that apply to you.

## Self-custody

Corium never holds your funds or your keys. Every trade, launch and claim is a transaction you sign in your own wallet and that executes on Solana. Transactions are final. Corium cannot reverse, cancel or refund them.

## Risk

Tokens launched here are created by users, have no intrinsic value and are highly volatile. You can lose everything you put in. Prices on a bonding curve move with every trade; slippage, partial fills and failed transactions can happen. Smart contracts, RPC providers, wallets and the Solana network can fail or behave unexpectedly.

## No advice, no endorsement

Nothing on Corium is financial, investment, legal or tax advice. A coin appearing on Corium, in rankings, in the sky or in chat is not an endorsement by Corium. Do your own research.

## Fees and bounties

Trades pay the fees shown in the trade panel. Part of Corium's fee on a coin in its final stretch funds that coin's supernova bounty, which is split by the published scoring rules. Bounty amounts are estimates until the distribution is published; the rules can change for coins launched after a change is announced.

## Your content

You are responsible for the coins you launch and everything you post, including names, images, descriptions, links and chat messages. Do not post anything illegal, infringing, sexually explicit, hateful or harassing, and do not impersonate people, projects or Corium. Do not use Corium to run scams, rug pulls, market manipulation, wash trading or bot spam. You give Corium permission to display what you post on Corium and in share images.

## Moderation

Corium may hide content, remove coins from its interface, and block wallets from chat or from the interface at any time, at its discretion. On-chain tokens and transactions are outside Corium's control and remain on Solana.

## Profiles

Signing in creates a public profile for your wallet with a generated name you can change. Your wallet address, profile, trades, launches and chat messages are public.

## Availability

Corium is provided as is, without warranties of any kind. It can be changed, interrupted or discontinued at any time.

## Liability

To the fullest extent allowed by law, Corium and its contributors are not liable for any loss arising from your use of Corium, including trading losses, failed transactions, bugs, or the acts of other users or of third-party protocols.

## Changes

We may update these terms. The version you agreed to is part of your signed sign-in message; a new version asks you to agree again the next time you sign in.

## What you sign

Signing in signs this statement with your wallet:

> Sign in to Corium. I confirm I am 18 or older and agree to the Corium Terms of Use v1.

The message also pins the exact text above by its SHA-256 hash:

`69b3d3c77e709f09e53efd76fe9a260e3d9a9eb222aae1bd4052e4d4f9a07875`

If the terms change, the version changes, and you are asked to agree again the next time you sign in.
